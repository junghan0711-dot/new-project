...
- 處理任何公司接案、政府計畫、營運、活動、標案、文件或內部管理工項時，不必等使用者另外說「整理知識」；開始前先查 `2026 Data base/知識庫/公司方法庫/index.md` 是否已有可複用方法，完成後主動做一次知識沉澱判斷。
- 每次工作完成時快速檢查：①是否可能跨兩案重複使用；②是否影響成本、品質、進度或客戶判斷；③是否形成新的決策邏輯、風險規則、檢核表、模板或 SOP；④是否為既有方法提供新證據或反例。符合任兩項，直接建立或更新公司方法頁，不需等使用者提醒。
- 尚未驗證的內容標為「想法」或「試行」，不得寫成公司標準；有重大爭議、涉及敏感資料或會改變正式作業規範時，先請使用者確認。
- 當次事實與決策回寫對應 `專案-*.md` 或工項決策紀錄；跨案原則寫入 `知識庫/公司方法庫/`；可直接操作的格式放入 `Templates/` 或 `創作庫/`。三者必須以 wikilink 雙向串聯。
- 新增或更新公司方法後，同步更新 `知識庫/index.md`、`知識庫/log.md`、方法頁的 `maturity`／`owner`／`updated`，並在對應專案頁補 `related_knowledge` 或進度紀錄。
- 簡單、一次性且無複用價值的行政動作只記專案進度，不為了形式製造知識頁；知識沉澱不能妨礙本次交付。
