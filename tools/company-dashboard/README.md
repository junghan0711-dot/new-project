...
- 週報 / 月報產生器：依目前總控台資料產出可複製或下載的會議、週報或月報摘要，可填報告期間與主管補充重點，內容包含整體狀態、各專案摘要、待查核、同仁回報、資料品質與建議下一步。
...
- 主管交辦中心：指定同事欄位會帶出目前各專案已出現過的同仁名單與 `同仁通訊錄` 名單；若該專案有 `同仁通訊錄`，送出交辦後會依姓名寄出 Email 通知，並支援像「玟樺」對應「陳玟樺」這類簡稱比對。
- 同仁通訊錄：讀取各專案 Google Sheet 的 `同仁通訊錄` 工作表，集中在總控台顯示；此頁籤只在 Google 帳號通過 `ALLOWED_EMAILS` 驗證後可查看。
- 修改歷程查詢：讀取各專案 Google Sheet 的 `修改歷程` 工作表，集中查詢工項回報、案件列管、案件進度與諮詢輔導場次的欄位異動，支援依專案、修改人、資料表、欄位與關鍵字篩選，並可複製目前清單。
